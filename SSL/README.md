...
This will generate the server certificate: localhost.crt. With that certificate and the server private key (localhost.private.key) you can configure the web server.
This will also generate a dummyRootCA.srl file with a serial number which will be increased with each certification generated by the dummyRootCA authority. 

Alternatively you could use the openssl CA command in conjunction with a ca.cnf file. You have an example in this repository (ca.cnf). First you should create the 
necessary files:
